...
ourBoard.once('wifiShield', (obj) => {
  ...
  ourBoard.connect(ip)
    .then(() => {
      ourBoard.wifiPost(ip, '/command', {'command': '~'}, (err) => {
        console.log(err);
        setTimeout(() => {
          ourBoard.wifiPost(ip, '/command', {'command': '5'}, (err) => {
            console.log(err);
            setTimeout(() => {
              ourBoard.wifiPost(ip, '/command', {'command': 'b'}, (err) => {
                console.log(err);
              });
            }, 20);
          });
      ...
      // ourBoard.wifiPost(ip, '/command', {'command': 'n'});
